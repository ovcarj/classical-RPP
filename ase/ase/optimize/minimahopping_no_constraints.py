...
#        self._Pb_positions = self._atoms.positions[self._Pb_indices]
#        self._Br_positions = self._atoms.positions[self._Br_indices]
        constraints_inorganic_Hookean = [Hookean(a1=int(self._Pb_indices[i]), a2=int(j), rt=self._atoms.get_distances(int(self._Pb_indices[i]), int(j), mic=True) + 0.1, k=10.) for i in range(len(self._Pb_indices)) for j in self._indices_list[i]]
...
#        self._atoms.set_constraint(constraints)
        self._atoms.set_constraint(constraints_inorganic_Hookean)
