...
from ase.calculators.lammpslib import is_upper_triangular, convert_cell
...
class MinimaHopping:
    ...
    _default_settings = {
        ...
        'pressure': 1.01325, # bar, pressure
        'taut': 0.5e3,  # fs, time constant for Berendsen temperature coupling
        'taup': 0.5e3,  # fs, time constant for Berendsen pressure coupling
        'compressibility': 0.001} # bar-1, the compressibility of the material
...
#        self._log('par')
    ...
    def _optimize(self):
        ...
        self._log('ene')
        tri_mat, coord_transform = convert_cell(self._atoms.get_cell())
        self._atoms.set_positions([np.matmul(coord_transform, position) for position in atoms.get_positions()])
        self._atoms.set_cell(tri_mat)
    ...
    def _molecular_dynamics(self, resume=None):
        ...
        traj = io.Trajectory('md%05i.traj' % self._counter, 'a',
                             self._atoms)
        dyn = NPT(self._atoms, timestep=self._timestep * units.fs, temperature=self._temperature, externalstress=1e-5, ttime=1000*units.fs, pfactor=0.05)
