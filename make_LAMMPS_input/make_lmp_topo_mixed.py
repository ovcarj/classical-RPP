...
prefix = 'tertBA'        #large organic molecule
...
N_Br = (3*N+1) / 2   #number of bromide atoms
N_I = (3*N+1) / 2   #number of iodide atoms

# Large organic molecule info (tertBA)
...
# Br and I indices needed for molecular indexing
...
# 3 in line below is for mixed

#file.write(str(info[4] + small_info[4] + 2) + ' atom types\n')
file.write(str(info[4] + small_info[4] + 3) + ' atom types\n')
...
for i in range(N_MOL*N_chem*N_A + N_mol*N_chem*N_B, len(atoms)):
    ...
    if (symbols[i] == 'I'):
        chg = -1.0
        typ = str(len(Masses) + len(small_Masses) + 3)
    if (symbols[i] != 'Pb' and symbols[i] != 'Br' and symbols[i] != 'I'):
        print('Wrong atom type found where Pb or Br or I expected, exiting.')
        sys.exit()
...
I_typ = str(info[4] + small_info[4] + 3)

#eps_ii = eps_ij**2 / eps_jj
#sigma_ii = 2 * sigma_ij - sigma_jj
...
for i in range(0, len(h_all_PairC)):
        eps = str(np.sqrt(h_all_PairC[i][1] * eps_I))
        sigma = str(0.5*(h_all_PairC[i][2] + sigma_I))
        file.write('pair_coeff      ' + str(int(h_all_PairC[i][0])) + ' ' + I_typ + ' ' + 'lj/cut/coul/long  ' + eps + ' ' + sigma + '\n')

file.write('\n')

buckNP = np.array((32690390.937995, 0.150947, 0.000000))
buckNB = np.array((94836.351975893, 0.3352375, 0.000000))
buckNI = np.array((112936.714213, 0.342426, 0.000000))

buckCP = np.array((32690390.937995, 0.150947, 0.000000))
buckCB = np.array((94836.351975893, 0.3352375, 0.000000))
buckCI = np.array((112936.714213, 0.342426, 0.000000))

buckPP_Br = np.array((74933300.5606326, 0.123246948356808, 0.000000))
buckPP_I = np.array((70359906.629702, 0.131258, 0.000000))
buckPP = np.array((np.sqrt(buckPP_Br[0] * buckPP_I[0]), np.sqrt(buckPP_Br[1] * buckPP_I[1]), np.sqrt(buckPP_Br[2] * buckPP_I[2]) ))

buckPB = np.array((110223.38165565, 0.302100469483568, 0.000000))
buckPI = np.array((103496.133010, 0.321737,  0.000000))

buckBB = np.array((24274.90558983, 0.45286103286385, 654.4127155))

buckII = np.array((22793.338582, 0.482217, 696.949542))
buckIB = np.array((np.sqrt(buckBB[0] * buckII[0]), np.sqrt(buckBB[1] * buckII[1]), np.sqrt(buckBB[2] * buckII[2]) ))
...
for N_typ in N_types:
     ...
     file.write('pair_coeff      ' + N_typ + ' ' + I_typ + ' '
             + 'buck/coul/long    ' + str(buckNI[0]) + ' ' + str(buckNI[1]) + ' ' + str(buckNI[2]) + '\n')
...
for C_typ in C_types:
    ...
    file.write('pair_coeff      ' + C_typ + ' ' + I_typ + ' '
            + 'buck/coul/long    ' + str(buckCI[0]) + ' ' + str(buckCI[1]) + ' ' + str(buckCI[2]) + '\n')
...
file.write('pair_coeff      ' + Pb_typ + ' ' + I_typ + ' '
        + 'buck/coul/long    ' + str(buckPI[0]) + ' ' + str(buckPI[1]) + ' ' + str(buckPI[2]) + '\n')

file.write('pair_coeff      ' + Br_typ + ' ' + Br_typ + ' '
        + 'buck/coul/long    ' + str(buckBB[0]) + ' ' + str(buckBB[1]) + ' ' + str(buckBB[2]) + '\n')

file.write('pair_coeff      ' + I_typ + ' ' + I_typ + ' '
        + 'buck/coul/long    ' + str(buckII[0]) + ' ' + str(buckII[1]) + ' ' + str(buckII[2]) + '\n')

file.write('pair_coeff      ' + Br_typ + ' ' + I_typ + ' '
        + 'buck/coul/long    ' + str(buckIB[0]) + ' ' + str(buckIB[1]) + ' ' + str(buckIB[2]) + '\n')
