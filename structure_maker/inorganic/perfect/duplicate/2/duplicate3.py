...
if(len(MA) > 0):
    ...
    MA_atoms_up.positions[:,2] += 29.

long_atoms_up = long_atoms.copy()
long_atoms_up.positions[:,2] += 29.

inorganic_up = inorganic.copy()
inorganic_up.positions[:,2] += 29.

#atoms = MA_atoms + MA_atoms_up + inorganic + inorganic_up
atoms = long_atoms + long_atoms_up + MA_atoms + MA_atoms_up + inorganic + inorganic_up

atoms.set_cell(frame.get_cell())
atoms.cell[2][2] += 29.

atoms.write('2n3_regular_super.traj')
